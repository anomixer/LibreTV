...
function addAdultAPI() {
    ...
    if (!HIDE_BUILTIN_ADULT_APIS && (localStorage.getItem('yellowFilterEnabled') === 'false')) {
        ...
        adultTitle.innerHTML = `黃色資源採集站 <span class="adult-warning">
...
function toggleSettings(e) {
    ...
    if (settingsPanel.classList.contains('show')) {
        ...
        // 只有設置了管理員密碼且未驗證時才攔截
        if (hasAdminPassword && !isVerified('ADMINPASSWORD')) {
            e.preventDefault();
            e.stopPropagation();
...
function resetSearchArea() {
    ...
    try {
        ...
        // 更新頁面標題
        document.title = `LibreTV - 免費在線視頻搜索與觀看平臺`;
    } catch (e) {
...
async function search() {
    ...
    try {
        ...
        if (yellowFilterEnabled) {
            const banned = ['倫理片', '福利', '裡番動漫', '門事件', '蘿莉少女', '制服誘惑', '國產傳媒', 'cosplay', '黑絲誘惑', '無碼', '日本無碼', '有碼', '日本有碼', 'SWAG', '網紅主播', '色情片', '同性片', '福利視頻', '福利片'];
